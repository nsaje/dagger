...
func TestParser(t *testing.T) {
	...
	expected := []AlarmDefinition{
		AlarmDefinition{
			LeafNode{"test", GT, 15.0, 0},
			"",
		},
		AlarmDefinition{
			BinNode{OR,
				LeafNode{"avg(cpu_util{t1=v1,t2=v2}, 5s)", GT, 30.0, 0},
				BinNode{AND,
					LeafNode{"avg(sum(test))", GT, 16.0, 0},
					LeafNode{"sum(test2)", LT, 13.0, 0}},
			},
			"",
		},
		AlarmDefinition{
		...
		AlarmDefinition{
			BinNode{OR,
				LeafNode{"avg(cpu_util{t1=v1,t2=v2}, 5s)", GT, 30.0, 0},
				BinNode{AND,
					LeafNode{"avg(sum(test{t1=v1}))", GT, 16.0, 0},
					LeafNode{"sum(test2)", LT, 13.0, 0}},
			},
			"t1",
		},
